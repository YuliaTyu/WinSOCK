...
{
	setlocale(LC_ALL, "");
	INT iResult = 0; //переменная нужня для отслеживания результатов выподнения функций
	//1) Инициализация WinSOCK
	WSADATA wsaData;
	iResult = WSAStartup(MAKEWORD(2, 2), &wsaData);//выбор версии WinSOCK
	if (iResult != 0)
	{
	...
	hints.ai_family = AF_INET;       //family семейство протоколов(стэк протоколов), INET - TCP /IPv4
	...
	//7)Разрываем соединение
	iResult = shutdown(connect_socket, SD_BOTH);
	if (iResult != 0)
		cout << "Shutdown failed with error: " << WSAGetLastError() << endl;

	//8)Освобождаем ресурсы WinSOCK
	closesocket(connect_socket);
	WSACleanup();
}